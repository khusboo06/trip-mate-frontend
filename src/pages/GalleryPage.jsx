...
export default function GalleryPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#0f172a] via-[#1e293b] to-[#faf8f6] px-3 sm:px-6 py-16 sm:py-20 mt-14 sm:mt-0 font-['Poppins']">
      <div className="relative w-full max-w-6xl px-3 sm:px-8 py-6 sm:py-8 rounded-2xl shadow-2xl bg-gradient-to-b from-[#0f172a] via-[#1e293b] to-[#0b1220] border border-[#d4af37]/30 backdrop-blur-lg">
        ...
        <form onSubmit={handleUpload} className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8 sm:mb-10">
          <input
            type="file"
            accept="image/*"
            onChange={(e) => setFile(e.target.files[0])}
            className="w-full sm:w-auto bg-[#faf8f6]/10 border border-[#d4af37]/40 rounded-xl px-4 py-2.5 text-[#f9f7e8] focus:ring-2 focus:ring-[#d4af37] outline-none"
          />
          <button
            type="submit"
            disabled={uploading}
            className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-[#d4af37] to-[#b8860b] text-[#0f172a] px-6 py-2.5 rounded-xl font-semibold shadow-md transition-all duration-300 disabled:opacity-70"
          >
            <Upload className="w-5 h-5" />
            {uploading ? "Uploading..." : "Upload"}
          </button>
        ...
        {images.length === 0 ? (
          <p className="text-center text-[#f9f7e8]/80 italic">
            No images yet. Be the first to upload!
          </p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 sm:gap-6">
            {images.map((img) => (
              <div
                key={img._id}
                className="relative group rounded-2xl overflow-hidden border border-[#d4af37]/30 bg-gradient-to-b from-[#1e293b]/80 to-[#0f172a]/90 shadow-lg transition-all duration-300"
              >
                <img
                  src={img.url}
                  alt="Trip"
